...
if(email&&password){
 const login=await assertOk(await request(`${backend}/trpc/system.login`,{method:"POST",headers:{"content-type":"application/json","x-trpc-source":"smoke"},body:JSON.stringify({json:{email,password}})}),"authenticated login");
 ...
 await assertOk(await request(`${backend}/trpc/intelligence.access`,{headers:{cookie,"x-trpc-source":"smoke"}}),"intelligence access");
