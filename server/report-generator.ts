...
import { audits, findings, actions, auditResponses, questions, referentials, processes, sites, evidenceFiles, users } from "../drizzle/schema";
...
async function fetchAuditData(auditId: number): Promise<AuditData> {
  ...
  const responses = await db
    ...
    .leftJoin(processes, eq(questions.processId, processes.id))
  ...
  const auditProcesses = processIds.length > 0
    ? await db.select().from(processes).where(inArray(processes.id, processIds))
    : [];
