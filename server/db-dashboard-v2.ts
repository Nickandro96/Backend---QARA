...
export async function getDashboardDrilldown(
  ...
  if (type === "findings") {
    ...
    const referentialData = referentialIds.length > 0
      ? await db.select().from(referentials).where(inArray(referentials.id, referentialIds as number[]))
      : [];
 ...
 */
export async function getDashboardSuggestions(userId: number, filters?: DashboardFilters) {
  const db = await getDb();
  if (!db) throw new Error("Database not available");
