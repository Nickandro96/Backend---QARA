...
const base = { aiAnalyzed: true, marketsImpacted: ["EU"], rolesImpacted: ["fabricant"], sourceRegistryId: "mdcg", analysisCriticality: "watch" as const, impactLevel: "High" };
...
test("un item action_required reste toujours visible quel que soit le profil", () => {
  assert.equal(isWatchItemVisible({ ...base, analysisCriticality: "action_required" }, { marketsImpacted: ["US"], rolesImpacted: ["importateur"], sourceIds: ["fda"] }), true);
});
...
test("le tri pertinent place action_required puis les analyses en attente", () => {
  assert.ok(
    watchPriority({ ...base, analysisCriticality: "action_required" }) <
