 ...
 * Usage: tsx server/seed-dashboard-simple.ts
 */
...
async function main() {
  ...
  const auditTypes = ["internal", "external", "supplier", "certification"] as const;
  const auditStatuses = ["draft", "in_progress", "completed", "closed"] as const;
  ...
  const findingStatuses = ["open", "in_progress", "closed", "verified"] as const;
  ...
  const actionTypes = ["corrective", "preventive", "improvement"] as const;
  const actionStatuses = ["open", "in_progress", "completed", "verified", "cancelled"] as const;
  ...
  const answerTypes = ["conforme", "nok", "na", "partial"] as const;
